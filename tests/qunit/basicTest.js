...
test('simple search', function() {
    ...
    Ember.run.begin();
    Ember.RSVP.all([
        model.create({content: {title: 'basic title 1', integer: 40}}).save(),
        model.create({content: {title: 'basic title 2', integer: 40}}).save(),
        model.create({content: {title: 'basic hello', integer: 40}}).save(),
    ]);
    Ember.run.end();
